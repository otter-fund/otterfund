...
// otterfund progress indicators — the one place the "fill sweeps from 0 on mount"
...
// to the live accent (via the --of-accent / --color-primary token) but accept an
...
export function ProgressBar({
  ...
  return (
    <div
      className={className}
      style={{ height, borderRadius: 999, background: "var(--color-of-track)", overflow: "hidden" }}
    >
      <div
        style={{
          height: "100%",
  ...
  return (
    <div className={className} style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: "rotate(-90deg)" }} aria-hidden="true">
        <circle cx={c} cy={c} r={r} fill="none" stroke="var(--color-of-track)" strokeWidth={stroke} />
        <circle
          cx={c}
          cy={c}
          r={r}
