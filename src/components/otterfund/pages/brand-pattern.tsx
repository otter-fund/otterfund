...
import { GuillochePattern, GuillocheSeal, braid, waveField } from "@/components/otterfund/guilloche";
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <Card className="relative col-span-full overflow-hidden p-6">
        ...
        <div className="relative">
          ...
          <div className="of-num mt-2 text-[40px] leading-none tracking-tight text-[var(--color-of-ink)]">
            {fmt(48213.55)}
          </div>
          <p className="mt-2 text-[13px] text-[var(--color-of-muted)]">Across 4 accounts · updated today</p>
        </div>
      </Card>
      ...
      <Card className="relative grid min-h-[180px] place-items-center overflow-hidden p-6 text-center">
        ...
        <div className="relative">
          <div className="text-[14px] font-semibold text-[var(--color-of-ink)]">No transactions yet</div>
          <p className="mx-auto mt-1 max-w-[220px] text-[13px] text-[var(--color-of-muted)]">
      ...
      <Card className="flex items-center gap-4 p-6">
        ...
        <div>
          <div className="text-[14px] font-semibold text-[var(--color-of-ink)]">Verified seal</div>
          <p className="mt-1 text-[13px] text-[var(--color-of-muted)]">
      ...
      <Card className="col-span-full overflow-hidden p-6">
        <div className="flex items-center gap-4">
          <span className="whitespace-nowrap text-[13px] font-semibold text-[var(--color-of-ink)]">Statement</span>
          ...
          <span className="of-num whitespace-nowrap text-[13px] text-[var(--color-of-muted)]">June 2026</span>
        </div>
        <p className="mt-3 text-[12px] text-[var(--color-of-muted)]">
  ...
  return (
    <div className="space-y-3">
      <svg
        ...
        className="w-full rounded-[14px] bg-[var(--color-of-surface)]"
        role="img"
...
function Engraving({ accent }: { accent: string }) {
  ...
  return (
    <div className="space-y-3">
      <canvas ref={canvasRef} className="w-full rounded-[14px] bg-[var(--color-of-surface)]" style={{ height: 240 }} />
      <div className="flex flex-wrap items-center gap-2">
        <input value={text} onChange={(e) => setText(e.target.value)} maxLength={6} className="of-field w-24" placeholder="$" />
        {["$", "€", "£", "B", "¥"].map((g) => (
...
function SealPlayground({
  ...
  return (
    <div className="space-y-3">
      <div className="grid place-items-center rounded-[14px] bg-[var(--color-of-surface)] py-8">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input value={seal.label} onChange={(e) => set("label", e.target.value)} maxLength={3} className="of-field w-20" placeholder="$" />
        {["$", "€", "£", "B", "✓"].map((g) => (
...
function RibbonPlayground({ accent, accentDeep }: { accent: string; accentDeep: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid place-items-center rounded-[14px] bg-[var(--color-of-surface)] py-6">
...
function ValuesBar({ values }: { values: Record<string, number | string | boolean> }) {
  ...
  return (
    <div className="flex items-center gap-2 pt-1">
      <code className="of-scroll flex-1 overflow-x-auto whitespace-nowrap rounded-lg bg-[var(--color-of-canvas)] px-2.5 py-1.5 text-[11px] text-[var(--color-of-muted)]">
        {json}
      </code>
      <Button
        variant="outline"
...
function Slider({
  ...
  return (
    <label className="block">
      <span className="flex justify-between text-[10.5px] font-semibold uppercase tracking-wide text-[var(--color-of-faint)]">
        {label} <span className="of-num text-[var(--color-of-ink)]">{value}</span>
      </span>
      <input
        type="range"
        min={min}
        ...
        className="mt-0.5 h-1 w-full accent-[var(--of-accent)]"
      />
    </label>
  );
}
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center gap-2 self-end pb-1 text-[12px] text-[var(--color-of-ink)]">
      <input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} className="accent-[var(--of-accent)]" />
      {label}
    </label>
  );
}
...
function Section({ n, title, desc, children }: { n: string; title: string; desc: string; children: React.ReactNode }) {
  return (
    <Card className="of-enter p-6">
      <div className="mb-5">
        <div className="flex items-baseline gap-3">
          <span className="of-num text-[15px] text-[var(--color-of-faint)]">{n}</span>
          <CardLabel>{title}</CardLabel>
        </div>
        <p className="mt-2 max-w-[70ch] text-[13px] leading-relaxed text-[var(--color-of-muted)]">{desc}</p>
      </div>
      {children}
    </Card>
  );
...
export function BrandPatterns() {
  ...
  return (
    <div className="mt-8 space-y-6" style={{ maxWidth: 1000, margin: "32px auto 0" }}>
      <header className="of-enter">
        <h2 className="text-[22px] tracking-tight" style={{ fontFamily: "var(--font-num)" }}>
          Guilloché &amp; engraving
        </h2>
        <p className="mt-1 text-[14px] text-[var(--color-of-muted)]">
      ...
      <Section
        ...
        desc="How the texture reads once it's applied to real otterfund surfaces: the reference for where and how to reach for it in the product. Everything below is a knob for tuning the pieces shown here."
      >
